...
type, public, extends(Type_File_Base):: Type_File_OFF_Options
  type(Type_XML_Tag)::            OFF_Options     !< Main XML tag.
...
contains
  ...
  elemental subroutine free_off_options(file_d,also_base)
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  associate(shl_out=>file_d%shl_out,sol_out=>file_d%sol_out,rst_out=>file_d%rst_out,prb_out=>file_d%prb_out)
    call file_d%get_stream(stream=stream)!; if (iostat/=0) return

    call file_d%OFF_Options%set(tag_name='OFF_Options',search_into_string=stream)

    call inputs%set( tag_name='inputs', search_into_other_tag=file_d%OFF_Options)
    call outputs%set(tag_name='outputs',search_into_other_tag=file_d%OFF_Options)
    ...
    call tag%set(tag_name='solver_file_name',search_into_other_tag=inputs)
    file_d%fn_solv = OS%string_separator_fix(string=trim(tag%tag_val%vs))
    call tag%set(tag_name='mesh_file_name',search_into_other_tag=inputs)
    file_d%fn_mesh = OS%string_separator_fix(string=trim(tag%tag_val%vs))
    call tag%set(tag_name='bc_file_name',search_into_other_tag=inputs)
    file_d%fn_bc = OS%string_separator_fix(string=trim(tag%tag_val%vs))
    call tag%set(tag_name='init_file_name',search_into_other_tag=inputs)
    file_d%fn_init = OS%string_separator_fix(string=trim(tag%tag_val%vs))
    call tag%set(tag_name='procmap_file_name',search_into_other_tag=inputs)
    ...
    call tag%set(tag_name='solution_file_name',search_into_other_tag=outputs)
    file_d%fn_sol = OS%string_separator_fix(string=trim(tag%tag_val%vs))
    call tag%set(tag_name='profiling_file_name',search_into_other_tag=outputs)
    file_d%fn_prof = OS%string_separator_fix(string=trim(tag%tag_val%vs))
    call tag%set(tag_name='shell_out_frequency',search_into_other_tag=outputs)
    shl_out = cton(str=tag%tag_val%vs,knd=1_I8P)
    call tag%set(tag_name='solution_out_frequency',search_into_other_tag=outputs)
    sol_out = cton(str=tag%tag_val%vs,knd=1_I8P)
    call tag%set(tag_name='solution_restart_frequency',search_into_other_tag=outputs)
    rst_out = cton(str=tag%tag_val%vs,knd=1_I8P)
    call tag%set(tag_name='probe_out_frequency',search_into_other_tag=outputs)
