!> @ingroup DerivedType
!> @{
!> @defgroup Data_Type_HashTCellDerivedType Data_Type_HashTCell
!> @}

!> @ingroup PublicProcedure
!> @{
!> @defgroup Data_Type_HashTCellPublicProcedure Data_Type_HashTCell
!> @}

!> @ingroup PrivateProcedure
!> @{
!> @defgroup Data_Type_HashTCellPrivateProcedure Data_Type_HashTCell
!> @}

!> @brief Module Data_Type_HashTCell contains the definition of Type_HashTCell type and useful procedures for its handling.
!> Type_HashTCell contains the cells data stored as a dynamic Hierachical Structure. The links data are stored by
...
!> @ingroup Data_Type_HashTCellDerivedType
...
!-----------------------------------------------------------------------------------------------------------------------------------
contains
  !> @ingroup Data_Type_HashTCellPrivateProcedure
  !> @{
  ...
  type(Type_Cell),   intent(IN)::    d    !< Data of the current link.
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  type(Type_HashID),            intent(IN)::  ID   !< ID (unique) of the current link.
  type(Type_Cell), allocatable, intent(OUT):: d    !< Data of the current link.
  !---------------------------------------------------------------------------------------------------------------------------------
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  class(Type_HashTCell),  intent(INOUT):: htable !< Hash table.
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  implicit none
  class(Type_HashTCell),        intent(IN)::  htable !< Hash table.
  type(Type_HashID),            intent(IN)::  ID     !< ID (unique) of the current link.
  type(Type_Cell), allocatable, intent(OUT):: d      !< Data of the current link.
  integer(I4P)::                              bucket !< Bucket index of the current link.
  !---------------------------------------------------------------------------------------------------------------------------------
