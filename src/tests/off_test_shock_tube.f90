...
call simulation%mesh%save_solution_into_file(file_name=trim(adjustl(outf_name)), off=.false., vtk=.true., ascii=.true.)
...
contains
  ...
  call cli%init(progname='off_test_shock_tube',                                &
                authors='S. Zaghi',                                            &
                help='Usage: ',                                                &
                examples=["off_test_shock_tube --output shock_tube_solution"], &
