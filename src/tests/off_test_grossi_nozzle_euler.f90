...
program off_test_grossi_nozzle_euler
!< OFF test: marco.
...
contains
  ...
  call cli%init(progname='off_test_marco',   &
                authors='M. Grossi',         &
                help='Usage: ',              &
                examples=["off_test_marco"], &
                epilog=new_line('a')//"all done")

  call cli%add(switch='--input',                                &
               help='input file name',                          &
               required=.false.,                                &
               def='./src/tests/grossi-nozzle-euler/marco.ini', &
               act='store')
...
endprogram off_test_grossi_nozzle_euler
