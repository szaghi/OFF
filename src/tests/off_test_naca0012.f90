!< OFF test: subsonic NACA0012 airfoil.

program off_test_naca0012
!< OFF test: subsonic NACA0012 airfoil.
...
contains
  ...
  call cli%init(progname='off_test_naca0012',   &
                authors='S. Zaghi',             &
                help='Usage: ',                 &
                examples=["off_test_naca0012"], &
                epilog=new_line('a')//"all done")

  call cli%add(switch='--input',                            &
               help='input file name',                      &
               required=.false.,                            &
               def='src/tests/naca0012_input/naca0012.ini', &
               act='store')
