...
if [ "$1" == "-clean" ] ; then
  ...
  ./IBM ibm_options.opt                    # running IBM with options defined in "ibm_options.dat"
  cp ./input/solver.opt ./output/          # copying solver options
  cp ./input/procmap-no_mpi.opt ./output/  # copyng procmap without MPI
  cp ./input/procmap-mpi.opt ./output/     # copyng procmap with    MPI
else
